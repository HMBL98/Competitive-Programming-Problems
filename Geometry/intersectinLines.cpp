...
bool LineasIguales(const Linea& l, const Linea& m) {
	return LineasParalelas(l, m) && EQ( 0, ( l.p - l.q ).Cruz( l.p - m.q ) )&& EQ( 0, ( m.q - m.p ).Cruz( m.q - l.p ) );
}
...
int InterseccionSegmentos(const Linea& s, const Linea& t) {
  ...
  return (ManoDerecha(t.p, t.q, s.p) != ManoDerecha(t.p, t.q, s.q))? 1: 0;
}
...
int main(){
  ...
  while(casos--){
    ...
    Linea l1(p1,p2);
    Linea l2(p3,p4);
    int inter = InterseccionRectas(l1,l2);
    if(inter == 1){
	Punto inter = PuntoInterseccion(l1,l2);
