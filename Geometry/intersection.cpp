...
int main(){
	...
	while(n--){
		Punto l1,l2,c1,c4;
		cin >> l1.x >> l1.y >> l2.x >> l2.y >> c1.x >> c1.y >> c4.x >> c4.y;
		Linea linea(l1,l2);
		Punto c3,c2;
		bool cinco;
		if(c1.x < c4.x && c1.y > c4.y){
			Punto c33(c1.x,c4.y);
			Punto c22(c4.x,c1.y);
			c3 = c33;
			c2 = c22;
                	cinco = (c3.x <= linea.p.x && c2.x >= linea.p.x && c3.y <= linea.p.y && c2.y >= linea.p.y);
		}
		if(c1.x > c4.x && c1.y < c4.y){
			Punto c33(c4.x,c1.y);
			Punto c22(c1.x,c4.y);
...
//			cinco = ()
		...
		int uno = InterseccionSegmentos(linea,lineaC1);
		int dos = InterseccionSegmentos(linea,lineaC2);
		int tres = InterseccionSegmentos(linea,lineaC3);
		int cuatro = InterseccionSegmentos(linea,lineaC4);
		if(uno == 1 || uno == -1 || dos == 1 || dos == -1 || tres == 1 || tres == -1 || cuatro == 1 || cuatro == -1 || cinco){
			cout << "T" << endl;
		}else{
			cout << "F" << endl;
		}
